...
let productos = JSON.parse(localStorage.getItem('ranchillo_pc_productos')) || [
    // Perros
...
function enviarWhatsApp() {
    ...
    let texto = `*🐾 RANCHILLO VENTAS SISTEMA APPLE - REPORTE DE CAJA DIARIO 🐾*\n`;
...
function aplicarEstilosTema() {
    ...
    const paletteKey = localStorage.getItem('ranchillo_pc_palette') || 'naranja';
...
function cambiarPaleta(key) {
    ...
    localStorage.setItem('ranchillo_pc_palette', key);
    aplicarEstilosTema();
...
function setTemaManual(newTheme) {
    ...
    localStorage.setItem('ranchillo_pc_theme', newTheme);
    aplicarEstilosTema();
...
function toggleFondoPerrito() {
    ...
    localStorage.setItem('ranchillo_pc_bg_image', isBgImg ? 'true' : 'false');
...
function inicializarTema() {
    const savedTheme = localStorage.getItem('ranchillo_pc_theme') || 'light';
    document.documentElement.setAttribute('data-theme', savedTheme);
    aplicarEstilosTema();
    actualizarIconosTema(savedTheme);

    // Inicializar Fondo de Perrito
    const savedBgImg = localStorage.getItem('ranchillo_pc_bg_image') === 'true';
    if (savedBgImg) {
